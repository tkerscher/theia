...
/**
 * Frank-Tamm formula (photon count)
 *
 *     d^3 N         alpha  /       1   \
 * -------------- = ------- | 1 - ----- |
 *  dx dlam dPhi     lam^2  \      n^2  /
 *
 * where alpha is the fine structure constant
 *
 * Note that we assume particle travels at light speed (beta = 1).
*/
float frank_tamm_count(float lambda, float n) {
    float cos_theta = 1.0 / n;
    float sin2_theta = (1.0 - cos_theta) * (1.0 + cos_theta);
    ...
    float result = 7.2973525693 / (lambda*lambda) * sin2_theta;
    //result will be negative for n < 1.0 -> no radiation (zero contrib)
    return max(result, 0.0);
}
/**
 * Frank-Tamm formula (energy)
 *
 *     d^3 N                                  1    /       1   \
 * -------------- = 0.5 * c^2 * e * mu_0 * ------- | 1 - ----- |
 *  dx dlam dPhi                            lam^3  \      n^2  /
 *
 * Difference from pi*c^2*e^2*mu_0 found in literature stems from energy in eV
 * and dphi (radial)
*/
float frank_tamm_eV(float lambda, float n) {
    float cos_theta = 1.0 / n;
    float sin2_theta = (1.0 - cos_theta) * (1.0 + cos_theta);
    //for better numerical stability, convert lambda from nm -> µm (closer to one)
    lambda *= 1e-3;
    float result = 9.04756408986352 / (lambda * lambda * lambda) * sin2_theta;
    //result will be negative for n < 1.0 -> no radiation (zero contrib)
    return max(result, 0.0);
}

//switch Frank-Tamm between photon count and energy
#ifdef FRANK_TAMM_USE_ENERGY
#define frank_tamm frank_tamm_eV
#else
#define frank_tamm frank_tamm_count
#endif
