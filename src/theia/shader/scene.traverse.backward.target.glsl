#ifndef _INCLUDE_SCENE_TRAVERSE_BACKWARD_TARGET
#define _INCLUDE_SCENE_TRAVERSE_BACKWARD_TARGET
...
#ifndef SCENE_TRAVERSE_BACKWARD_TARGET_DISABLE_MIS
...
*/
void createResponse(
    BackwardRay ray,                    ///< Ray that generated that hit
...
ResultCode processHit(
    ...
    bool isSrc = (hit.flags & MATERIAL_LIGHT_SOURCE_BIT) != 0;
    ...
    if (allowResponse && isSrc && hit.customId == targetId) {
...
void processShadowRay(
    ...
    bool isSrc = (hit.flags & MATERIAL_LIGHT_SOURCE_BIT) != 0;
    if (!hit.valid || hit.customId != targetId || !isSrc) return; //hit something else
    //propagate ray to hit
...
*/
void traceShadowRay(
    BackwardRay ray,                ///< Ray the shadow one is based on
