...
def sampleData1D():
    ...
    grad = -np.exp(-x)*(np.exp(x)+np.sin(4*np.pi*x)-4*np.pi*np.cos(4*np.pi*x))
...
def test_sampleTable1D(sampleData1D):
    ...
    sample = theia.lookup.sampleTable1D(data, 10, boundary=(5.,8.))[1:]
...
def test_sampleTable2D(sampleData2D):
    ...
    sample = theia.lookup.sampleTable2D(data, 100, 100, boundaries=(None, (3.0,8.0)))[2:]
